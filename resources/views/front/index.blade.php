...
@section('tittle', 'ryobi sewa barang')
@section('content')
<main class="max-w-[640px] mx-auto min-h-screen flex flex-col relative has-[#Bottom-nav]:pb-[144px]">
    ...
    <section id="Categories" class="flex flex-col gap-[10px] mt-[30px] px-5">
        ...
        <div class="grid grid-cols-3 gap-4">
            ...
            @empty
            <p>Belum ada kateogri terbaru</p>
            @endforelse
        </div>
    </section>
    ...
    <section id="Recommendation" class="flex flex-col gap-[10px] mt-[30px] px-5">
        ...
        <div class="flex flex-col gap-5">
            @forelse ($random_products as $item_random_products)
            <a href="{{ route('front.details', $item_random_products->slug) }}" class="card">
                <div class="flex items-center gap-3">
                    <div class="w-20 h-20 flex shrink-0 rounded-2xl overflow-hidden bg-[#F6F6F6] items-center">
                        <div class="w-full h-[50px] flex shrink-0 justify-center">
                            <img src="{{asset(Storage::url($item_random_products->thumbnail))}}" class="h-full w-full object-contain" alt="thumbnail" />
                        </div>
                    </div>
                    <div class="w-full flex flex-col gap-1">
                        <p class="font-semibold">{{ $item_random_products->name }}</p>
                        <div class="flex items-center justify-between">
                            <p class="text-sm leading-[21px] text-[#6E6E70]">Rp {{ number_format($item_random_products->price,0,',','.') }}/day</p>
